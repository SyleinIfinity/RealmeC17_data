IF EXISTS (SELECT * FROM sys.databases WHERE name = N'svKHANH')
	BEGIN
		USE master
		ALTER database svKHANH set single_user with rollback immediate
		DROP DATABASE svKHANH;
	END
GO
create database svKHANH
go
use [svKHANH]
