...
SELECT HO + ' ' + TEN AS HO_TEN, DIACHI,
		YEAR(NGAYLAMVIEC) AS NAM_BAT_DAU_LAM_VIEC
FROM NHANVIEN;
...
--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--Tuần 11--
/*
	Câu 1: Hiển thị địa chỉ, số điện thoại nhà cung cấp có tên giao dịch [VINAMILK] là gì?
*/
select * from NHACUNGCAP

Update NHACUNGCAP
Set TENGIAODICH = case MACONGTY
		When 'CTY00001' then N'Sữa Vinamilk'
		When 'CTY00004' then N'Kem Vinamilk'
		When 'CTY00008' then N'Phô mai Vinamilk'
		When 'CTY00011' then N'Sữa chua Vinamilk'
		When 'CTY00019' then N'Nước giải khát Vinamilk'
		When 'CTY00014' then N'Đường Vinamilk'
END
go
Select ncc.MACONGTY, ncc.DIACHI, ncc.DIENTHOAI
From NHACUNGCAP ncc
Where TENGIAODICH like N'% Vinamilk'

/*
	Câu 2: Loại hàng thực phẩm do những công ty nào cung cấp và địa chir cửa các công ty đó là gì?
*/
Update LOAIHANG
Set TENLOAIHANG= case MALOAIHANG
		When 'LH000004' then N'Thực phẩm'
		else TENLOAIHANG
END

Select mh.MAHANG, mh.LOAIHANG_NO, ncc.DIACHI
From NHACUNGCAP ncc, MATHANG mh, LOAIHANG lh
Where ncc.MACONGTY = mh.CONGTY_NO
AND mh.LOAIHANG_NO = lh.MALOAIHANG
AND lh.TENLOAIHANG = N'Thực phẩm'


/*
	Câu 3: Những khách hàng nào(tên giao dịch) đã đặt mua mặt hàng sữa hộp XYZ của công ty?
*/
UPDATE NHACUNGCAP
SET TENCONGTY = N'Công ty VINAMILK'
WHERE MACONGTY IN('CTY00013', 'CTY00015', 'CTY00019');

UPDATE KHACHHANG
SET TENCONGTY = N'Công ty VINAMILK'
WHERE MAKHACHHANG IN('KH000015','KH000013', 'KH000018');
UPDATE KHACHHANG
SET TENGIAODICH = N'Sữa hộp XYZ'
WHERE MAKHACHHANG IN('KH000015', 'KH000013', 'KH000018');
GO
select MAKHACHHANG, k.TENCONGTY
from KHACHHANG k, DONDATHANG dh, 
	CHITIETDATHANG ct, MATHANG mh, NHACUNGCAP ncc
where k.MAKHACHHANG = dh.KHACHHANG_NO
AND dh.SOHOADON = ct.SOHOADON
AND ct.MAHANG = mh.MAHANG
AND mh.CONGTY_NO = ncc.MACONGTY
AND k.TENCONGTY = ncc.TENCONGTY
AND k.TENGIAODICH = N'Sữa hộp XYZ'

/*
	Câu 4: Những đơn đặt hàng nào yêu cầu giao hàng ngay tại công ty đặt hàng và những đơn đó là của công ty nào?
*/

Update DONDATHANG
Set NOIGIAOHANG =  N'045 Đường Trần Quốc Toản, TP. HCM'
	Where SOHOADON = 'SHD00010' 


Select dh.SOHOADON, dh.NOIGIAOHANG, ncc.TENCONGTY
from KHACHHANG k, DONDATHANG dh, 
	CHITIETDATHANG ct, MATHANG mh, NHACUNGCAP ncc
where k.MAKHACHHANG = dh.KHACHHANG_NO
AND dh.SOHOADON = ct.SOHOADON
AND ct.MAHANG = mh.MAHANG
AND mh.CONGTY_NO = ncc.MACONGTY
AND dh.NOIGIAOHANG =ncc.DIACHI

/*
	Câu 5: Tổng số tiền mà khách hàng phải trả cho mỗi đơn đặt hàng là bao nhiêu?
*/
Update MATHANG
set SOLUONG = case
	When SOLUONG is null then 80
	else SOLUONG
END,
	GIAHANG = case
	When GIAHANG is null then 1254500
	else GIAHANG
END;
Go
UPDATE CHITIETDATHANG
SET MAHANG = CASE SOHOADON
    WHEN 'SHD00009' THEN 'MH000008'
    WHEN 'SHD00006' THEN 'MH000010'
    WHEN 'SHD00008' THEN 'MH000008'
	else MAHANG
END;
...
UPDATE DONDATHANG
SET KHACHHANG_NO = 'KH000003'
WHERE SOHOADON IN ('SHD00004');
go
SELECT c.SOHOADON, k.MAKHACHHANG,
    SUM((c.GIABAN * c.SOLUONG) - (c.GIABAN * c.SOLUONG * c.MUCGIAMGIA)) AS 'Tổng tiền'
FROM DONDATHANG d, KHACHHANG k, CHITIETDATHANG c
WHERE d.KHACHHANG_NO = k.MAKHACHHANG
	AND d.SOHOADON = c.SOHOADON
GROUP BY c.SOHOADON, k.MAKHACHHANG;