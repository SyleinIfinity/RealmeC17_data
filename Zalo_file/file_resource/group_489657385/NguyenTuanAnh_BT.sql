...
UPDATE KHACHHANG
SET TENGIAODICH = N'Giao dịch thứ 1'
WHERE MAKHACHHANG IN('KH000004');
GO
...
SELECT MAHANG, TENHANG,
    (SELECT TENCONGTY 
     FROM NHACUNGCAP 
     WHERE MACONGTY = M.CONGTY_NO) AS TENCONGTY
FROM MATHANG M;
...
UPDATE CHITIETDATHANG
SET SOLUONG = CASE MAHANG
    WHEN 'MH000001' THEN 0
    WHEN 'MH000002' THEN 0
END
WHERE MAHANG IN ('MH000001', 'MH000002');
SELECT MAHANG, TENHANG
FROM MATHANG
WHERE MAHANG IN (
    SELECT MAHANG
...
SELECT K.MAKHACHHANG, K.TENCONGTY,
    (SELECT SUM(C.GIABAN * C.SOLUONG)
     FROM DONDATHANG D, CHITIETDATHANG C, KHACHHANG K
     WHERE D.SOHOADON = C.SOHOADON AND D.KHACHHANG_NO = K.MAKHACHHANG) AS TONG_TIEN
FROM KHACHHANG K
GROUP BY K.MAKHACHHANG, K.TENCONGTY
HAVING (SELECT COUNT(*)
         FROM DONDATHANG D
         WHERE D.KHACHHANG_NO = K.MAKHACHHANG) > 0;
--6. Hãy cho biết tổng số tiền lời mà công ty thu  được từ mỗi mặt hàng trong năm 2022.
SELECT 
    M.MAHANG,
    M.TENHANG,
    (SELECT SUM(C.SOLUONG * C.GIABAN)
     FROM CHITIETDATHANG C
     WHERE C.SOHOADON IN (
         SELECT D.SOHOADON
         FROM DONDATHANG D
         WHERE D.NGAYGIAOHANG BETWEEN '2022-01-01' AND '2022-12-31'
     ) AND C.MAHANG = M.MAHANG) AS DOANHTHU
FROM 
    MATHANG M
WHERE 
    M.MAHANG IN (
...
ORDER BY 
    DOANHTHU DESC;
--Câu 3
--Sửa lại nội dung trong bảng KHACHHANG
UPDATE KHACHHANG
SET TENGIAODICH = CASE 
    WHEN MAKHACHHANG = 'KH000001' THEN N'Giao dịch thứ 01'
    WHEN MAKHACHHANG = 'KH000002' THEN N'Giao dịch thứ 02'
    WHEN MAKHACHHANG = 'KH000003' THEN N'Giao dịch thứ 03'
    WHEN MAKHACHHANG = 'KH000004' THEN N'Giao dịch thứ 04'
    WHEN MAKHACHHANG = 'KH000005' THEN N'Giao dịch thứ 05'
    WHEN MAKHACHHANG = 'KH000006' THEN N'Giao dịch thứ 06'
    WHEN MAKHACHHANG = 'KH000007' THEN N'Giao dịch thứ 07'
    WHEN MAKHACHHANG = 'KH000008' THEN N'Giao dịch thứ 08'
    WHEN MAKHACHHANG = 'KH000009' THEN N'Giao dịch thứ 09'
    WHEN MAKHACHHANG = 'KH000010' THEN N'Giao dịch thứ 10'
    WHEN MAKHACHHANG = 'KH000011' THEN N'Giao dịch thứ 11'
    WHEN MAKHACHHANG = 'KH000012' THEN N'Giao dịch thứ 12'
    WHEN MAKHACHHANG = 'KH000013' THEN N'Giao dịch thứ 13'
    WHEN MAKHACHHANG = 'KH000014' THEN N'Giao dịch thứ 14'
    WHEN MAKHACHHANG = 'KH000015' THEN N'Giao dịch thứ 15'
    WHEN MAKHACHHANG = 'KH000016' THEN N'Giao dịch thứ 16'
    WHEN MAKHACHHANG = 'KH000017' THEN N'Giao dịch thứ 17'
    WHEN MAKHACHHANG = 'KH000018' THEN N'Giao dịch thứ 18'
    WHEN MAKHACHHANG = 'KH000019' THEN N'Giao dịch thứ 19'
    WHEN MAKHACHHANG = 'KH000020' THEN N'Giao dịch thứ 20'
END;
GO
