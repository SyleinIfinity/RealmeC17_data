...
CREATE OR ALTER PROCEDURE sp_GhiNhanKetQuaHienMau
    ...
    @maNV_LayMau INT,
    @theTich INT,
    @ghiChu NVARCHAR(500) = NULL  -- Tham số tùy chọn
AS
BEGIN
    ...
    BEGIN TRY
        ...
        -- Kiểm tra 1.1: Phiếu sàng lọc có tồn tại không?
        IF @trangThaiSangLoc IS NULL
        BEGIN
            SET @ErrorMessage = N'Lỗi: Không tìm thấy phiếu sàng lọc #' + CAST(@maPhieuSangLoc AS NVARCHAR(10));
            THROW 50001, @ErrorMessage, 1;
        END
        
        -- Kiểm tra 1.2: Kết quả sàng lọc có ĐẠT không?
        IF @trangThaiSangLoc <> N'Đạt'
        BEGIN
            SET @ErrorMessage = N'Lỗi: Phiếu sàng lọc #' + CAST(@maPhieuSangLoc AS NVARCHAR(10)) 
                              + N' có trạng thái "' + @trangThaiSangLoc + N'". Chỉ chấp nhận trạng thái "Đạt".';
            THROW 50002, @ErrorMessage, 1;
        END
        
        -- ============================================
        -- BƯỚC 2: VALIDATE - Kiểm tra đã hiến chưa (tránh duplicate)
        -- ============================================
        IF EXISTS (
            SELECT 1 
            FROM PHIEUHIENMAU 
        ...
        BEGIN
            SET @ErrorMessage = N'Lỗi: Phiếu sàng lọc #' + CAST(@maPhieuSangLoc AS NVARCHAR(10)) 
                              + N' đã được ghi nhận hiến máu rồi (Duplicate entry).';
        ...
        -- ============================================
        -- BƯỚC 4: VALIDATE - Kiểm tra nhân viên tồn tại
        -- ============================================
        IF NOT EXISTS (
            SELECT 1 
            FROM NHANVIENYTE 
            WHERE maNhanVienYTe = @maNV_LayMau
        )
        BEGIN
            SET @ErrorMessage = N'Lỗi: Không tìm thấy nhân viên y tế #' + CAST(@maNV_LayMau AS NVARCHAR(10));
        ...
        -- ============================================
        INSERT INTO PHIEUHIENMAU (
            thoiGianHien,       -- Thời gian hiện tại
            theTich,            -- Thể tích đã validate
            maNV_LayMau,        -- Nhân viên thực hiện
            maPhieuSangLoc      -- Liên kết với phiếu sàng lọc
        )
        VALUES (
            GETDATE(),          -- Lấy thời gian server (UTC+7)
            @theTich,
            @maNV_LayMau,
            @maPhieuSangLoc
        );
        ...
        SET @maPhieuHienMauMoi = SCOPE_IDENTITY();
        ...
        PRINT N'  - Mã phiếu hiến máu: ' + CAST(@maPhieuHienMauMoi AS NVARCHAR(10));
        PRINT N'  - Thể tích: ' + CAST(@theTich AS NVARCHAR(10)) + N' ml';
        PRINT N'  - Thời gian: ' + CONVERT(NVARCHAR(30), GETDATE(), 120);
        
        -- Trả về mã phiếu mới cho application
        SELECT @maPhieuHienMauMoi AS MaPhieuHienMauMoi;
...
BEGIN
    ...
    -- ============================================
    -- BƯỚC 3: CẬP NHẬT TRẠNG THÁI Phiếu Đăng Ký
    -- ============================================
    -- Đánh dấu phiếu đăng ký này đã hoàn tất toàn bộ quy trình
    UPDATE PHIEUDANGKY
    SET trangThai = N'Hoàn thành'
    WHERE maDangKy = @maDangKy;
    
    PRINT N'✓ Đã cập nhật trạng thái Phiếu Đăng Ký #' + CAST(@maDangKy AS NVARCHAR(10)) + N' → "Hoàn thành"';
