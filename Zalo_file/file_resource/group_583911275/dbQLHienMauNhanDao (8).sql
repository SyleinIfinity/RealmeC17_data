...
-- 2. TRIGGER CHẶN XÓA PHIẾU HIẾN MÁU (CHỐNG MẤT MÁU)

CREATE TRIGGER trg_ChanXoaPhieuHienMau
ON PHIEUHIENMAU
FOR DELETE
AS
BEGIN
    RAISERROR (N'LỖI NGHIÊM TRỌNG: Không được phép xóa Phiếu Hiến Máu. Dữ liệu này là tài sản kho.', 16, 1);
    ROLLBACK TRANSACTION;
END;
GO

-- =======================================================
-- 3. TRIGGER CHẶN SỬA DỮ LIỆU NHẠY CẢM TRÊN PHIẾU HIẾN

IF OBJECT_ID('trg_BaoVeDuLieuPhieuHienMau') IS NOT NULL
    DROP TRIGGER trg_BaoVeDuLieuPhieuHienMau;
GO

CREATE TRIGGER trg_BaoVeDuLieuPhieuHienMau
ON PHIEUHIENMAU
FOR UPDATE
AS
BEGIN
    -- Chặn sửa thể tích (Chống gian lận kho)
    IF UPDATE(theTich)
    BEGIN
        RAISERROR (N'LỖI: Không được phép sửa thể tích máu đã nhập kho.', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END

    -- Chặn sửa liên kết hồ sơ (Chống tráo người/nhóm máu)
    IF UPDATE(maPhieuSangLoc)
    BEGIN
        RAISERROR (N'LỖI NGHIÊM TRỌNG: Không được phép thay đổi mã phiếu sàng lọc.', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END

    -- Chặn sửa thời gian (Chống gian lận KPI)
    IF UPDATE(thoiGianHien)
    BEGIN
         RAISERROR (N'LỖI: Không được phép thay đổi thời gian hiến máu.', 16, 1);
         ROLLBACK TRANSACTION;
         RETURN;
    END
END;
...
-- 4. TRIGGER CHẶN SỬA PHIẾU SÀNG LỌC

CREATE TRIGGER trg_ChanSuaSangLocKhiDaHienMau
ON PHIEUSANGLOC
FOR UPDATE
AS
BEGIN
    ...
    BEGIN
        ...
        IF UPDATE(trangThai)
        BEGIN
            RAISERROR (N'LỖI LOGIC: Người này đã thực hiện lấy máu. Không thể thay đổi kết quả sàng lọc (Đạt/Không đạt) vào lúc này. Vui lòng xử lý hủy túi máu trước.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        IF UPDATE(nhomMau)
        BEGIN
            RAISERROR (N'NGUY HIỂM: Đã thu nhận máu. Không được phép sửa "Nhóm máu" để tránh nhầm lẫn tai hại khi truyền máu.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
    END
